...
import { generateToken, verifyToken } from "../token/tokenHandler.js";
...
export const fetchUserByEmailOrID = async (data, isEmail = true) => {
  ...
  if (!isEmail) sql = "SELECT `id`, `name`, `email`, `created_at`, `updated_at`, `points` FROM `users` WHERE `id`=?";
...
export default {
  ...
  login: async (req, res, next) => {
    try {
      ...
      const [result] = await DB.execute("INSERT INTO `refresh_tokens` (`user_id`,`token`) VALUES (?,?)", [user.id, md5Refresh]);
  ...
  refreshToken: async (req, res, next) => {
    try {
      ...
      const [refTokenRow] = await DB.execute("SELECT * from `refresh_tokens` WHERE token=?", [md5Refresh]);
      ...
      const [result] = await DB.execute("UPDATE `refresh_tokens` SET `token`=? WHERE `token`=?", [newMd5Refresh, md5Refresh]);
